...
async function generateUniqueTransactionCode(supabase: Awaited<ReturnType<typeof createClient>>) {
  for (let i = 0; i < 20; i += 1) {
    const code = random8DigitCode();
    const { data, error } = await supabase.from('payments').select('id').eq('transaction_code', code).limit(1);
...
export async function POST(req: Request) {
  try {
    ...
    const baseUrl = process.env.PAYMENT_BASE_URL || 'https://jobly.az';
    ...
    const payload = {
      ...
      result_url: `${baseUrl}/payment/result`,
    };
    ...
    if (!res.ok) {
      return NextResponse.json({ message: json?.message ?? 'ePoint request failed' }, { status: 502 });
    }
    ...
    if (!redirectUrl) {
      return NextResponse.json({ message: json?.message ?? 'redirect_url not found' }, { status: 502 });
    }

    const { data: userRow } = await supabase.from('users').select('email').eq('user_id', user.id).maybeSingle();

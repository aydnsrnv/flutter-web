...
function StatBox({
  ...
  return (
    <div className="flex min-w-0 flex-1 flex-col items-center justify-center">
      <div className="text-center text-[18px] font-bold text-foreground">{value}</div>
      <div className="mt-2 grid h-11 w-11 place-items-center rounded-xl" style={{ backgroundColor: 'rgba(36, 91, 235, 0.10)' }}>
        {icon}
      </div>
      <div className="mt-2 text-center text-[13px] font-medium text-muted-foreground">{label}</div>
    </div>
  );
}
...
export default function CompanyViewPage() {
  ...
  const isUuid = useMemo(() => /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(companyKey), [companyKey]);
  ...
  const loadCompany = useCallback(async () => {
    ...
    try {
      ...
      setCompanyError(t('company_details_error').replace('{error}', e?.message ?? String(e)));
      setCompany(null);
      setCompanyUuid(null);
    } finally {
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      const rows = (data ?? []) as { view_count?: number | null; applied_count?: number | null }[];
      const views = rows.reduce((sum, j) => sum + (Number(j.view_count) || 0), 0);
      const applied = rows.reduce((sum, j) => sum + (Number(j.applied_count) || 0), 0);
  ...
  const loadMoreJobs = useCallback(
    async (nextOffset: number) => {
      ...
      try {
        const { data, error } = await supabase
          .from('jobs')
          .select('id, job_number, title, company_id, company_name, company_logo, city, create_time, min_salary, max_salary, view_count, applied_count')
          .eq('status', true)
          .eq('company_id', companyIdForJobs as any)
        ...
        setJobsError(t('company_jobs_load_error').replace('{error}', e?.message ?? String(e)));
        setHasMore(false);
      } finally {
  ...
  if (companyError) {
    return <div className="rounded-2xl border border-border bg-card p-4 text-sm text-muted-foreground">{companyError}</div>;
  }
  ...
  const jobsCount = company.job_count != null && company.job_count > 0 ? company.job_count : jobs.length;
  ...
  return (
    <div className="flex flex-col gap-4">
          ...
          <div className="mx-auto h-[88px] w-[88px] overflow-hidden rounded-full bg-muted">
            {logo ? (
              ...
              <div className="grid h-full w-full place-items-center">
                <Building size={44} variant="Linear" className="text-muted-foreground" />
              </div>
            )}
          </div>

          <div className="mt-4 text-center text-[22px] font-bold text-foreground">{name}</div>

          <div className="mt-4 flex w-full items-center justify-between">
            <StatBox
              icon={<Eye size={24} variant="Linear" color="var(--jobly-main, #245BEB)" />}
              value={formatCount(totalViews).replace('{k}', t('number_k_suffix'))}
              label={t('company_stats_views')}
            />
            <StatBox
              icon={<UserTick size={24} variant="Linear" color="var(--jobly-main, #245BEB)" />}
              value={formatCount(totalApplied).replace('{k}', t('number_k_suffix'))}
              label={t('company_stats_applied')}
            />
            <StatBox
              icon={<Briefcase size={24} variant="Linear" color="var(--jobly-main, #245BEB)" />}
              value={formatCount(Number(jobsCount ?? 0)).replace('{k}', t('number_k_suffix'))}
              ...
              <div className="mt-2 flex justify-center">
                ...
                >
                  <ArrowDown2
                    ...
                    style={{ transform: aboutExpanded ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 160ms ease' }}
                  />
                </button>
              </div>
            )}
      ...
      <div className="mx-4 mb-2 mt-4">
        <h2 className="text-[18px] font-bold text-foreground">{t('active_jobs')}</h2>
      </div>
      ...
      <div className="mx-4 overflow-hidden rounded-lg bg-background">
        {initialJobsLoading ? (
          ...
          <div className="px-4 py-4 text-sm text-muted-foreground">{jobsError}</div>
        ) : jobs.length === 0 ? (
          <div className="px-4 py-6 text-center text-[16px] text-foreground">{t('no_active_job_for_company')}</div>
        ) : (
          <>
            <FlutterJobListGroup jobs={jobs} />
            <div ref={sentinelRef} className="h-px w-full" />
            {jobsLoading && hasMore && jobs.length > 0 ? (
              <div className="px-4 py-4 text-center text-sm text-muted-foreground">{t('updating')}</div>
            ) : null}
          </>
        )}
      </div>

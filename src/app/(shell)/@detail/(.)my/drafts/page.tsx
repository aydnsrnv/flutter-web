...
export default async function MyDraftsInterceptPage() {
  ...
  const { data, error } = await supabase
    .from(tableName)
    .select('id, title, city, company_name, company_logo, min_salary, max_salary, create_time')
    .eq('creator_id', uid)
    ...
    <div className="flex flex-col">
      <div className="mt-4">
        {error ? (
          ...
          <div className="rounded-lg bg-background px-4 py-6 text-center text-[16px] text-foreground">{t('noDraftsFound')}</div>
        ) : (
          <div className="space-y-3">
            {drafts.map((draft) => (
              <DraftItem key={String(draft.id)} draft={draft} draftType={draftType} />
            ))}
          </div>
        )}
      </div>
      ...
      <div className="mt-4 text-center">
        <p className="text-[12px] text-muted-foreground">{t('draftsInfoFooter')}</p>
      </div>
    </div>
  );
}

...
export default function FavoritesPage() {
  ...
  const fetchFavorites = useCallback(async () => {
    ...
    try {
      if (isJobsTab) {
        const ids = uniqueStrings(safeParseStringList(localStorage.getItem('jobly_fav_jobs')));
        if (ids.length === 0) {
          setJobs([]);
          return;
        }
        ...
        const rows = ([...(byId.data ?? []), ...(byJobNumber.data ?? [])] as JobRow[]).map((r) => ({
          id: String(r.id),
        ...
        const ids = uniqueStrings(safeParseStringList(localStorage.getItem('jobly_resume_favorites')));
        if (ids.length === 0) {
          setResumes([]);
          return;
        }
        ...
        const { data, error } = await supabase
          .from('resumes')
          .select('id, resume_number, full_name, desired_position, desired_salary, city, birth_year, experience_key, education_key, experiences, avatar, view_count, create_time, is_premium')
          .in('id', uuidIds)
          .order('create_time', { ascending: false });
      ...
      setError(t('favorites_load_error').replace('{error}', e?.message ?? String(e)));
    } finally {
      setLoading(false);
    }
  ...
  const filteredJobs = useMemo(() => {
    ...
    return jobs.filter((j) => {
      ...
      return title.includes(q) || company.includes(q) || city.includes(q) || number.includes(q);
    });
  }, [jobs, query]);
  ...
  const filteredResumes = useMemo(() => {
    ...
    return resumes.filter((r) => {
      ...
      return name.includes(q) || pos.includes(q) || city.includes(q) || number.includes(q);
    });
  }, [query, resumes]);
      ...
      {loading ? (
        ...
        filteredJobs.length === 0 ? (
          <div className="py-6 text-center text-sm text-muted-foreground">
            {jobs.length === 0 ? t('favorites_empty_jobs') : t('favorites_search_not_found')}
          </div>
        ) : (
        ...
        <div className="py-6 text-center text-sm text-muted-foreground">
          {resumes.length === 0 ? t('favorites_empty_resumes') : t('favorites_search_not_found')}
        </div>
      ) : (

...
import { MyResumesClient } from '@/app/(shell)/my/resumes/my-resumes-client';
    ...
    <div className="flex flex-col">
      ...
      <div className="mt-[6px]">
        {error ? (
          ...
          <div className="rounded-lg bg-background px-4 py-6 text-center text-[16px] text-foreground">{t('resumes_empty')}</div>
        ) : (
          <MyResumesClient initialResumes={rows} initialHasMore={rows.length >= limit} limit={limit} isActive={isActive} />
        )}
      </div>
    </div>
  );

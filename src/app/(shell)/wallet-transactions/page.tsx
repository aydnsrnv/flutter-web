...
export default async function WalletTransactionsPage() {
  ...
  const items = (Array.isArray(data) ? (data as WalletTxRow[]) : []).filter(Boolean);
  const limit = 20;
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="overflow-hidden rounded-2xl border border-border">
        {items.length === 0 ? (
          ...
          <WalletTransactionsClient initialItems={items} initialHasMore={items.length >= limit} limit={limit} />
        )}
      </div>

...
function formatTimeAndPostDate(iso: string, t: (k: string) => string) {
  ...
  if (startOfD.getTime() === startToday.getTime())
    return `${hhmm}, ${t("today")}`;
  if (startOfD.getTime() === startYesterday.getTime())
    return `${hhmm}, ${t("yesterday")}`;
  ...
  const load = useCallback(
    async ({ refresh }: { refresh: boolean }) => {
      ...
      try {
        ...
        const list = (
          Array.isArray(notifs) ? (notifs as NotificationRow[]) : []
        ).filter(Boolean);
  ...
  return (
    <div className="flex flex-col gap-4">
      {loading && rows.length === 0 ? (
        ...
        <EmptyState label={t("notifications_none")} />
      ) : (
          ...
          {rows.map((n, idx) => {
            ...
            return (
              <div key={n.id}>
                ...
                {!isLast ? (
                  <div className="h-[0.8px] w-full bg-black/10 dark:bg-white/10" />
                ) : null}
              </div>
            );
          })}
          ...
          {hasMore ? (
            <div className="p-3">
              <InfiniteScrollSentinel
                onVisible={() => void load({ refresh: false })}

...
function localizedExperienceTextFromResume(resume: ResumePopularItemData, t: (key: string) => string) {
...
export function ResumePopularItem({ resume }: { resume: ResumePopularItemData }) {
  const { t } = useI18n();
  const href = resume.resume_number != null ? `/resume/${resume.resume_number}` : `/resumes/${resume.id}`;
  const desiredPosition = resume.desired_position ?? t('resume_detail_position_not_specified');
  const expTextRaw = localizedExperienceTextFromResume(resume, t);
  const experienceText = expTextRaw && expTextRaw.trim() && expTextRaw.trim() !== '-' ? expTextRaw : '';

  const hasAvatar = !!resume.avatar && (resume.avatar.startsWith('http://') || resume.avatar.startsWith('https://'));
  ...
  return (
    ...
    >
      {hasAvatar ? (
        <img src={resume.avatar ?? ''} alt={resume.full_name} className="absolute inset-0 h-full w-full object-cover" loading="lazy" />
      ) : (
        <div className="absolute inset-0 grid place-items-center" style={{ backgroundColor: 'rgba(36,91,235,0.06)' }}>
      ...
      <div
        ...
        style={{
          background: 'linear-gradient(to bottom, rgba(36,91,235,0.06) 0%, rgba(36,91,235,0.70) 100%)',
        }}
      />

      <div className="absolute bottom-0 left-0 right-0 px-3 pb-3 pt-10">
        <div className="truncate text-[15px] font-bold text-white" style={{ lineHeight: 1.15 }}>
          {resume.full_name}
        </div>
        {experienceText ? (
          <div className="mt-[3px] truncate text-[12px] font-medium" style={{ color: 'rgba(255,255,255,0.92)' }}>
            {experienceText}
          </div>
        ) : null}
      </div>

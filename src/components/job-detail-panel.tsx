...
function SalaryText({
  ...
  return (
    <div className="inline-flex items-center gap-1">
      <span>{String(min).trim()} - {String(max).trim()}</span>
      <span className="text-[14px] font-bold" style={{ color: 'var(--jobly-main)' }}>{currencySymbol}</span>
    </div>
  );
}
...
function ContactTile({
  ...
  return (
    ...
    >
      ...
      <div className="shrink-0" style={{ color: 'var(--muted-foreground)', opacity: 0.45 }}>
...
function DetailRow({ title, value }: { title: string; value?: string | null }) {
  ...
  return (
    <div className="flex items-start gap-3 py-1">
      <div className="min-w-[44%] text-[14px] font-semibold" style={{ color: 'var(--foreground)' }}>
        {title}:
      </div>
      <div className="flex-1 text-[13px]" style={{ color: 'var(--muted-foreground)' }}>
        {value}
      </div>
    </div>
  );
...
function CopyableContactRow({
  ...
  return (
    <button type="button" onClick={onCopy} className="flex w-full items-center gap-2 text-left">
      <span className="shrink-0" style={{ color: 'var(--jobly-main)' }}>
        {icon}
      </span>
      <span className="min-w-0 flex-1 truncate font-semibold" style={{ color: 'var(--foreground)' }}>
        {value}
      </span>
      <span className="shrink-0" style={{ color: 'var(--muted-foreground)', opacity: 0.35 }}>
...
function DetailSection({ title, content }: { title: string; content?: string | null }) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-4">
      ...
      <div className="mt-3 grid gap-2 text-[14px]" style={{ color: 'var(--muted-foreground)' }}>
        {lines.map((raw, idx) => {
          ...
          return (
            <div key={idx} className="flex items-start gap-2">
              <div className="mt-[6px] h-[6px] w-[6px] rounded-full" style={{ backgroundColor: 'var(--jobly-main)' }} />
  ...
  return (
    ...
    >
      {src ? (
        ...
        <div className="text-[28px] font-bold" style={{ color: 'var(--muted-foreground)' }}>
...
export function JobDetailPanel({ job }: { job: JobDetailPanelData }) {
  ...
  const ageRange = (job.min_age != null || job.max_age != null)
    ? `${job.min_age ?? ''}-${job.max_age ?? ''}`.replace(/^\-/, '').replace(/\-$/, '')
    : null;
  ...
  const salaryNode = <SalaryText min={job.min_salary} max={job.max_salary} currencySymbol={manatSymbol} />;
  ...
  return (
    <div>
      <div className="relative rounded-t-2xl">
        ...
        >
          <div className="flex justify-end gap-3 p-4">
            <button
              ...
              style={{ backgroundColor: reported ? 'rgba(255,165,0,0.30)' : 'rgba(255,255,255,0.20)' }}
              onClick={openReport}
            >
          ...
          <div className="absolute bottom-4 left-4 flex items-center gap-[10px]">
            <div className="inline-flex items-center gap-1 rounded-2xl px-[10px] py-[6px]" style={{ backgroundColor: 'rgba(255,255,255,0.20)' }}>
        ...
        <div className="absolute -bottom-[45px] right-5">
          <div
            ...
            style={{ background: 'linear-gradient(135deg, #3a8bff 0%, #5ec6fa 100%)' }}
          >
            {job.company_id ? (
              <Link href={`/company/${String(job.company_id)}`} aria-label={job.company_name}>
                <CompanyLogoCircle src={job.company_logo} alt={job.company_name} />
              </Link>
            ) : (
              <CompanyLogoCircle src={job.company_logo} alt={job.company_name} />
            )}
      ...
      <div className="px-4 pb-24 pt-[60px]">
        ...
        <div className="mt-1 text-[14px]" style={{ color: 'var(--muted-foreground)' }}>
          {job.company_name}
        </div>
        {cityLabel ? (
        ...
        <div className="mt-3 rounded-xl" style={{ backgroundColor: 'rgba(36, 91, 235, 0.10)' }}>
          <div className="px-3 py-3">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <Money size={20} variant="Linear" color="var(--jobly-main)" />
                <div className="text-[14px] font-semibold" style={{ color: 'var(--muted-foreground)' }}>
                  {t('salary')}
                </div>
              </div>
              <div className="text-[16px] font-bold" style={{ color: 'var(--foreground)' }}>
            ...
            {jobTypeLabel ? (
              <div className="mt-2 flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <DocumentText size={20} variant="Linear" color="var(--jobly-main)" />
                  <div className="text-[14px] font-semibold" style={{ color: 'var(--muted-foreground)' }}>
                    {t('jobType')}
                  </div>
                </div>
                <div className="text-[16px] font-medium" style={{ color: 'var(--foreground)' }}>
                  {jobTypeLabel}
                </div>
              </div>
            ) : null}
        ...
        <div className="mt-4 rounded-2xl border border-border bg-card p-4">
          <div className="grid">
            <DetailRow title={t('jobNumber')} value={job.job_number != null ? String(job.job_number) : null} />
          ...
          <div className="mt-3 grid gap-2">
            <CopyableContactRow icon={<Message size={18} variant="Linear" />} value={job.mail ?? null} />
            <CopyableContactRow icon={<Call size={18} variant="Linear" />} value={job.number ?? null} />
          </div>
        </div>

        <div className="mt-4">
          <DetailSection title={t('jobRequirements')} content={job.request ?? null} />
        </div>
                ...
                <div className="rounded-t-2xl" style={{ backgroundColor: 'var(--background)' }}>
                  <div className="px-3 pb-3">
                    ...
                    <div className="mt-3 grid gap-[10px]">
                      ...
                      {job.mail && job.mail.trim() ? (
                        <ContactTile
                          icon={<Message size={25} variant="Linear" color="var(--jobly-main)" />}
                          title={t('email')}
                          subtitle={job.mail}
        ...
        ? createPortal(
            <div className="fixed inset-0" style={{ zIndex: 10000 }}>
              ...
              <div className="absolute left-1/2 top-1/2 w-[92%] max-w-[520px] -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-card p-4">
                ...
                <div className="mt-3 text-center text-[12px]" style={{ color: 'var(--muted-foreground)' }}>
                ...
                {!reported ? (
                  <div className="mt-4">
                    ...
                    />
                    {reportError ? (
                      <div className="mt-2 text-[12px] font-semibold" style={{ color: '#EF4444' }}>
                        {reportError}
                      </div>
                    ) : null}
                  </div>
